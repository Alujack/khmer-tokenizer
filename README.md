...
# The JavaScript/WASM bindings (from a checkout, until published to npm)
...
│   └── tests/          #   pytest suite
└── wasm/               # khmer-tokenizer for npm — wasm-bindgen/wasm-pack
...
and browsers, with TypeScript definitions generated by wasm-pack:
...
- **Publish the WASM bindings to npm** — the `wasm/` crate is built and
  tested (see "JavaScript / WASM usage" above); the `khmer-tokenizer` name
  is confirmed available on npm, pending an npm account/release step.
