...
- [x] **WASM bindings** — built as `wasm/` (2026-07-02): a
      ...
      package build). Crate is named `khmer-tokenizer` so the generated
      npm package matches the PyPI name; the name is confirmed available
      on npm — actually publishing there (npm account + `wasm-pack
      publish`) is the remaining step.
