...
- [x] **WASM bindings** — built as `wasm/` (2026-07-02): a
      wasm-bindgen/wasm-pack crate, detached from the Cargo workspace
      exactly like `py/`, exposing the same surface as the Python bindings
      (`new KhmerTokenizer({ words, strategy, frequencies, normalization })`,
      `segment`, `contains`, `size`, plus `splitKcc`/`normalize`/`isKhmer`),
      with TypeScript definitions generated by wasm-pack. 10-test
      wasm-bindgen-test suite runs in a real wasm engine under Node, plus
      its own CI job (clippy for wasm32 + `wasm-pack test --node` +
      package build). Publishes to npm as **`kh-tokenizer`** — npm's
      typosquat rule rejected `khmer-tokenizer` as "too similar to
      existing package khmertokenizer" (a third-party 2023 package), so
      the npm name diverges from PyPI/crates.io; the crate is named
